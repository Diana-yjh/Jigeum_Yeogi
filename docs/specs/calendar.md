...
### 0. 자녀가 2명 이상일 때 — 통합 달력 + 아이별 색
필터 없이 한 달력에 모두 표시한다. 아이마다 `AppColors.childColor(index)`(코랄·세이지·더스티 블루·더스티 플럼 순환)를 배정한다.
- **요약 카드**: `● 이름 N회`를 아이 수만큼 한 줄에(Wrap). 이 카드가 색 범례를 겸한다 — 별도 범례는 두지 않는다(중복).
- **날짜 셀**: 등원한 아이 수만큼 지름 4 도트를 아이 색으로 나란히(최대 4개, 간격 3). 도트가 없어도 높이 4는 유지.
- **상세 카드**: 날짜 아래 아이별 블록 — `● 이름 … 정규/보충 pill` / 타임바(막대 색 = 아이 색 35%) / `10시 · N시간 체류 · 20시`. 블록 사이 헤어라인 구분선. 등원한 아이만 표시.
- 월 기록(`childMonthRecordsProvider`)은 자녀 전체가 한 번에 오므로 날짜별 **리스트**로 모은다(맵에 한 건만 넣으면 형제 기록이 덮어쓴다).
- 자녀가 한 명이면 범례·이름 없이 아래 단일 레이아웃 그대로.
