...
### 0. 자녀 선택 칩 (자녀가 2명 이상일 때만)
타이틀 아래, 월 헤더 위에 가로 스크롤 칩 행. 선택된 아이는 `primary` 채움 + 흰 글자, 나머지는 `chipNeutral`.
- 월 기록(`childMonthRecordsProvider`)은 자녀 전체가 한 번에 오므로 **선택한 아이의 `studentId`로 걸러서** 그린다. 거르지 않으면 같은 날 두 아이 기록이 한 칸에서 덮어쓴다.
- 아이를 바꾸면 선택 날짜를 초기화해 그 아이 기준으로 자동 선택(오늘/마지막 출석일)한다.
- 상세 카드 날짜 앞에 `김테스트 · 8월 27일 목요일`처럼 이름을 붙인다. 자녀가 한 명이면 칩도 이름도 표시하지 않는다.
