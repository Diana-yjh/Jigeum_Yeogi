...
### 2.7 선생님 아래 반 아이 관리
각 선생님 행 아래에 **그 반에 속한 아이들**을 들여쓰기(↳) 행으로 나열한다.
- 아이 행의 ✕ = **반에서 빼기**. 다른 반에도 있는 아이는 그 수강 문서만 삭제(그 반 기록도 함께). **유일한 반이면 삭제 대신 연결 해제**(`unlinkStudent`, teacherCode='') — 아이와 지난 기록이 우리 아이 목록에 남는다. 규칙: 학부모는 본인 자녀의 teacherCode를 빈 값으로만 변경 가능.
- 마지막 행 "이 반에 아이 추가" → 다이얼로그: **기존 아이는 드롭다운으로 선택**(같은 아이 이름을 다시 입력하지 않는다) + "새로운 아이 직접 입력…" 옵션. 같은 반 중복 추가는 차단. **연결 해제된 수강(빈 코드)이 있는 아이를 고르면 새 문서 대신 그 문서를 재연결**(`relinkStudent`) — 지난 기록이 이어진다.
...
**같은 이름은 한 번만** 표시한다(수강이 여러 반이어도). 캡션에는 소속 선생님 닉네임들을 ' · '로 합쳐 보여준다. 연필 = 이름 수정(모든 반의 수강 문서 일괄 rename), 휴지통 = **모든 반에서 삭제**(일괄, 확인 다이얼로그). 반 단위 추가/빼기는 선생님 섹션에서 한다. **선생님이 2명 이상이면** 아이 이름 옆에 소속 선생님 닉네임을 캡션으로 표시한다. `teacherCode`가 빈 아이(선생님이 내보냄)는 항상 "연결된 선생님 없음" 캡션을 표시하고, 파생 선생님 목록에 빈 코드는 넣지 않는다. 아이 추가 다이얼로그는 선생님이 여럿이면 드롭다운으로 선택, 하나면 캡션으로 안내.
