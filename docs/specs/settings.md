...
- 행: 🏫 아이콘 + 닉네임 + 코드(모노, 탭=복사) + 연필(닉네임 수정, `EditNameDialog`) + 🔗해제.
...
아이가 여러 명이면 아이 행이 반복된다. **선생님이 2명 이상이면** 아이 이름 옆에 소속 선생님 닉네임을 캡션으로 표시한다. 아이 추가 다이얼로그는 선생님이 여럿이면 드롭다운으로 선택, 하나면 캡션으로 안내.
