...
- **탭별 요구사항 문서화**: `docs/specs/<탭>.md` + "탭 작업 전 필독" 규칙(CLAUDE.md). `docs/Design.md` 디자인 시스템 문서.
- **로그인 우선 진입**: 역할 선택 후 로그인 화면으로, 계정 없으면 회원가입 링크.
- **설정 개편**: 역할 분기(학부모=프로필/우리 아이/알림 토글/기타/로그아웃, 선생님=내 정보+반 코드+학생 수). 알림 설정 Firestore 저장, 선생님 코드 복사.

### 디자인
- **소프트 파스텔 & 큰 라운드 리뉴얼**: 채도 낮춘 코랄 + 웜 뉴트럴 + 세이지 보조색, radius 24, 소프트 그림자(AppDecoration), 그라디언트 히어로, 세그먼트 토글, G마켓 산스 폰트, 글씨 +12% 확대.
- **학부모 홈/달력 재설계**: 홈=오늘 상태 히어로(등하원 타임라인)+이번 주 도트. 달력=커스텀 그리드(도트·월 이동·체류 타임바). 선생님·학부모 홈 폴리시.

### 출석 화면 개선
- 오늘 스케줄 학생만 정규 표시, **오늘 예정 아님=회색 카드 + 등원 버튼 비활성**.
- **1시간 단위 시간대 그룹핑**(예: "오후 3시 · 2명").
- **하원 시 확인 다이얼로그**, 하원 완료 탭 시 기록 초기화 확인.

### 수정(버그/UX)
- 스케줄 요일 삭제가 저장 안 되던 문제(merge→update).
- 글씨 확대로 달력 셀 오버플로("bottom") → rowHeight 지정.
- 달력에서 세로 스크롤 막힘 → table_calendar 제스처 horizontalSwipe로 제한.

### 데이터 관리
- 요청으로 Firebase `attendance` 컬렉션(모든 출석 기록) 전체 삭제 이력 있음.
...
lib/features/calendar/
  parent_calendar_screen.dart       # 학부모 달력 (달력 탭)
  teacher_calendar_screen.dart      # 선생님 달력 (출석 화면 달력 아이콘 진입)
  widgets/calendar_cells.dart       # 한글 요일 헤더 + 날짜 셀(등원/오늘/선택)
lib/core/util/time_format.dart      # monthKey 추가
```
- 기존 placeholder `calendar_screen.dart` 삭제 → parent_calendar_screen로 대체

### 기능
- **학부모 달력**: 등원한 날=연한 오렌지, 오늘=진한 오렌지. 날짜 탭 → 등원→하원 타임라인, 총 체류 시간, "평소보다 N분 늦게/일찍 하원"(그 달 평균 하원 시각 대비, 기록 2개 이상일 때).
- **선생님 달력**: 등원 있던 날 표시. 날짜 탭 → 그 날 등원 학생 명단·시각·인원수. (출석 화면 우상단 달력 아이콘으로 진입 — 선생님 탭엔 달력 탭이 없으므로)
- 월 이동 시 해당 월 데이터로 실시간 갱신.

### 데이터
- 월별 provider 2종(childMonthRecords/teacherMonthRecords, "yyyy-MM" 키), 저장소 `teacherRecordsBetween` 추가.
- 기존 복합 인덱스(`teacherCode+date`, `parentUid+date`) 재사용 — 신규 배포 불필요.

### 남은 것
- "출석 예정" 표시는 `scheduledDays` 데이터 미수집으로 보류(현재 과거 기록 조회 중심).
...
- `firestore.rules`에서 chats/messages 규칙 삭제 후 재배포
- 시스템 메시지 칩(등하원 자동 삽입) 계획도 함께 폐기

## 열어둔 구조 / 다음
- 선생님이 외부 앱으로 초대 링크 전송 — 구조만 열어둠
- 선생님 "학생 추가/관리" 화면 + scheduledDays(출석 예정) 입력
- FCM 토큰 저장 필드는 있으나 실제 푸시는 Phase 6
- **Phase 6**: FCM 푸시 (등원/하원 시 학부모 알림)
